...
#### `outputCharts(component)`
Define custom charts for the component. This method should be implemented in the component's event code.

```python
def outputCharts(component):
    # Create and return a list of charts
    charts = []

    # Add a processing time chart
...
Define custom tables for the component. This method should be implemented in the component's event code.

```python
def outputTable(component):
    # Create and return a list of tables
    tables = []

    # Add a container metrics table
...
#### `run_ml_model(model_name, input_data, model_dir=None)`
...
This method supports various ML frameworks including scikit-learn, TensorFlow, PyTorch, and models saved with joblib or pickle. See the [ML Integration Guide](ml_integration.md) for more details.
...
These methods are available in all component types and are used for creating visualizations of simulation data.

### Chart Creation

#### `create_line_chart(header, description, series_data, y_axis_label=None, x_axis_label=None, cols=None, rows=None)`
...
#### `create_bar_chart(header, description, series_data, y_axis_label=None, x_axis_label=None, vertical=True, stacked=False, cols=None, rows=None)`
...
#### `create_pie_chart(header, description, data, cols=None, rows=None)`
...
#### `create_card(header, description, value, prefix=None, suffix=None, cols=None, rows=None)`
...
#### `create_table(data, title=None, description=None, column_order=None, column_labels=None, exclude_columns=None)`
...
#### `get_processing_times(csv_scraper=None)`
...
#### `get_action_counts(csv_scraper=None)`
...
#### `get_queue_length_timeline(csv_scraper=None)`
...
#### `get_container_metrics(csv_scraper=None)`
...
These methods are available in all component types and are used for integrating machine learning models into your simulation components.
...
This method supports various ML frameworks:
...
For more details on using machine learning in your components, see the [ML Integration Guide](ml_integration.md).
...
6. **Visualization**: Use the visualization methods to create meaningful charts and tables for your component's data.

7. **Machine Learning**: When using ML models, always check if the result is `None` before using it, and provide fallback logic in case the model fails to run.
