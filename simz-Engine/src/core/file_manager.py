...
class FileManager:
    ...
    def __init__(self, comp_reg_dir: str = None, base_dir: str = None):
        """
        ...
        """
        self.component_reg = (
            Path(comp_reg_dir) if comp_reg_dir else Path.cwd() / "component_Register"
        )
        self.base_dir = Path(base_dir) if base_dir else Path.cwd() / "projects"
        self.base_dir.mkdir(exist_ok=True, parents=True)
        self._create_component_reg_dir()

    def _create_component_reg_dir(self) -> None:
        self.component_reg.mkdir(exist_ok=True, parents=True)
        # create comp_reg.json
        data = {"path": str(self.component_reg), "components": []}

        component_reg_path = self.component_reg / "compSchema"
        component_reg_path.mkdir(exist_ok=True, parents=True)

        with open(self.component_reg / "config.json", "w") as f:
    ...
    def create_run(self, project_name: str, run_name: Optional[str] = None) -> str:
        ...
        run_id = (
            run_name
            or f"run_{uuid.uuid4().hex[:8]}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
    ...
    def get_reg_comp_config(self) -> Dict:
        with open(self.component_reg / "config.json", "r") as f:
            return json.load(f)

    def update_reg_comp_config(self, config: Dict) -> None:
        with open(self.component_reg / "config.json", "w") as f:
            json.dump(config, f, indent=4)
    ...
    def register_component(
        ...
        # Save Python file
        with open(self.component_reg / f"{component_name}.py", "w") as f:
            f.write(python_code)

        # Save schema
        with open(json_comp_path / f"{component_name}.json", "w") as f:
            json.dump(schema, f, indent=4)

        # Update project config
        config = self.get_reg_comp_config()
        config["components"].append(
            {"name": component_name, "created_at": datetime.now().isoformat()}
        )
    ...
    def get_component(self, project_name: str, component_name: str) -> Dict[str, Any]:
        ...
        if not component_path.exists():
            raise ValueError(
                f"Component '{component_name}' does not exist in project '{project_name}'."
            )
