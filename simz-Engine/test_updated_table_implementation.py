#!/usr/bin/env python3
"""
Test script for the updated table implementation.

This script tests:
1. Creating a table with the updated Table class
2. Adding table data to SimOutput
3. Verifying that table data is properly serialized to JSON
4. Ensuring the table structure matches the TypeScript interface
"""
...
TEST_DIR = Path("test_updated_table_implementation_output")
if TEST_DIR.exists():
    shutil.rmtree(TEST_DIR)
TEST_DIR.mkdir(parents=True, exist_ok=True)

def test_table_creation():
    """Test creating a table with the updated Table class."""
    ...
    table = Table.create_table(
        data=data,
        title="Test Table",
        description="A test table for the updated implementation",
        column_order=["id", "name", "type", "value"],
        column_labels={
            "id": "ID",
            "name": "Name",
            "type": "Type",
    ...
    assert "columnOrder" in table
    assert "columnLabels" in table
...
def test_component_table():
    ...
    assert "columnLabels" in component_table
    assert "columnOrder" in component_table
...
def test_metrics_table():
    ...
    assert "columnLabels" in metrics_table
    assert "columnOrder" in metrics_table
