...
def main():
    ...
    parser.add_argument('--host', type=str,
                        help='Host address to bind to (overrides SIMZ_HOST)')
    parser.add_argument('--port', type=int,
                        help='Port to listen on (overrides SIMZ_PORT)')
    parser.add_argument('--project-dir', type=str,
                        help='Directory where projects are stored (overrides SIMZ_PROJECT_DIR)')
    parser.add_argument('--debug', action='store_true',
                        help='Enable debug mode (overrides SIMZ_DEBUG)')
    ...
    print(f"Starting Socket.IO server on {env_manager.get('HOST')}:{env_manager.get('PORT')}")
